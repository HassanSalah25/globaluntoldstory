...
<div class="max-w-2xl">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Edit Feature Highlight</h1>
    </div>

    <form action="{{ route('admin.feature-highlights.update', $featureHighlight) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <input type="text" name="icon"
                           value="{{ old('icon', $featureHighlight->icon ?? '') }}"
                ...
                <div>
                    ...
                    <select name="context"
                        ...
                        <option value="services" {{ old('context', $featureHighlight->context) === 'services' ? 'selected' : '' }}>Services</option>
                        <option value="about"    {{ old('context', $featureHighlight->context) === 'about'    ? 'selected' : '' }}>About</option>
                        <option value="why-us"   {{ old('context', $featureHighlight->context) === 'why-us'   ? 'selected' : '' }}>Why Us</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Sort Order</label>
                    <input type="number" name="sort_order"
                           value="{{ old('sort_order', $featureHighlight->sort_order ?? 0) }}"
                           class="border border-gray-300 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-red-500 focus:border-transparent text-sm">
                </div>
            </div>
        </div>
