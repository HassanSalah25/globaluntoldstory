...
<div x-data="{ deleteId: null, deleteUrl: '' }">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage portfolio projects</p>
        </div>
        <a href="{{ route('admin.portfolio.create') }}" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium inline-flex items-center gap-2">
    ...
    <form method="GET" class="mb-4 flex gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search projects..." class="border border-gray-300 rounded-lg px-3 py-2 w-64 focus:ring-2 focus:ring-red-500 focus:border-transparent text-sm">
        ...
        @if(request('search'))
        <a href="{{ route('admin.portfolio.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium">Clear</a>
        @endif
    </form>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-3">
                        @if($item->image_url)
                        <img src="{{ $item->image_url }}" alt="{{ $item->slug }}" class="w-[60px] h-[40px] object-cover rounded">
                        @else
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.portfolio.edit', $item) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-lg text-sm font-medium">Edit</a>
                            <form action="{{ route('admin.portfolio.toggle', $item) }}" method="POST" class="inline">
                                @csrf @method('PATCH')
                            ...
                            <button
                                @click="deleteId = {{ $item->id }}; deleteUrl = '{{ route('admin.portfolio.destroy', $item) }}'"
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-4 py-12 text-center text-gray-400">No portfolio projects found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="deleteId" x-cloak class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl p-6 max-w-sm w-full mx-4 shadow-xl">
            ...
            <p class="text-gray-600 mb-4">Are you sure you want to delete this project? This cannot be undone.</p>
